...
import main.app.usw_random_chat.data.TokenInterceptor
import main.app.usw_random_chat.data.dto.MessageDTO
...
class ChatViewModel @Inject constructor(
    ...
    private val _opponentUserProfile = MutableStateFlow(
        ProfileDTO(
            "",
            "",
            ""
    ...
    val userProfile: MutableStateFlow<ProfileDTO> = _userProfile
    ...
    fun connectChrome(context: Context, url: String) {
        val intent = Intent(Intent.ACTION_VIEW, Uri.parse(url)).apply {
            // Chrome을 명시적으로 지정 (Chrome 패키지명)
            setPackage(chromeUrl)
        }

        // Chrome이 설치되어 있는지 확인 후 실행
        if (intent.resolveActivity(context.packageManager) != null) {
            context.startActivity(intent)
        } else {
            // Chrome이 설치되지 않았을 경우 기본 브라우저로 열기
